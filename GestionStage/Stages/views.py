...
def dashboard_enseignant(request):
    ...
    context = {
        'nb_stages': Stage.objects.filter(enseignant=request.user).count(),
        'nb_etudiants': Etudiant.objects.filter(enseignant_referent=enseignant).count(),
        'nb_rapports': Rapport.objects.filter(stage__enseignant=request.user, statut='soumis').count(),
        'derniers_stages': Stage.objects.filter(enseignant=request.user).order_by('-date_debut')[:5]
    }
...
def mes_rapports(request):
    ...
    if request.user.role == 'etudiant':
        rapports = Rapport.objects.filter(stage__candidature__etudiant=request.user)
    else:
        rapports = Rapport.objects.all()
    
    if selected_statut:
        rapports = rapports.filter(statut=selected_statut)
    
    if selected_promotion and request.user.role != 'etudiant':
        rapports = rapports.filter(stage__candidature__etudiant__promotion=selected_promotion)
    
    if date_filter:
        today = timezone.now().date()
        if date_filter == 'today':
            rapports = rapports.filter(date_soumission__date=today)
        elif date_filter == 'week':
            start_week = today - timedelta(days=today.weekday())
            rapports = rapports.filter(date_soumission__date__gte=start_week)
        elif date_filter == 'month':
            rapports = rapports.filter(date_soumission__month=today.month, date_soumission__year=today.year)
    ...
    paginator = Paginator(rapports.order_by('-date_soumission'), 10)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    
    promotions = []
    if request.user.role != 'etudiant':
        promotions = Rapport.objects.filter(
            stage__candidature__etudiant__promotion__isnull=False
        ).values_list(
            'stage__candidature__etudiant__promotion', 
            flat=True
        ).distinct().order_by('stage__candidature__etudiant__promotion')
...
def liste_rapports(request):
    ...
    if request.user.role == 'etudiant':
        rapports = rapports.filter(stage__candidature__etudiant__user=request.user)
    elif request.user.role == 'enseignant':
        rapports = rapports.filter(stage__enseignant=request.user)

    if selected_statut in dict(Rapport.STATUT_CHOICES):
        rapports = rapports.filter(statut=selected_statut)
    ...
    if date_filter == 'today':
        ...
        rapports = rapports.filter(date_soumission__date__gte=today - timedelta(days=today.weekday()))
    elif date_filter == 'month':
        rapports = rapports.filter(date_soumission__month=today.month, date_soumission__year=today.year)
    ...
    if search_query:
        rapports = rapports.filter(
            ...
            Q(stage__candidature__etudiant__user__first_name__icontains=search_query) |
            Q(stage__candidature__offre__entreprise__nom__icontains=search_query) |
            Q(stage__intitule__icontains=search_query)
        )

    paginator = Paginator(rapports.order_by('-date_soumission'), 25)
    ...
    context = {
        ...
        'selected_statut': selected_statut,
        'promotions': Etudiant.objects.exclude(promotion__in=['', None])
                      .order_by('promotion').values_list('promotion', flat=True).distinct(),
        'selected_promotion': selected_promotion,
        'date_filter': date_filter,
        'search_query': search_query,
    }
    
    return render(request, 'stages/liste_rapports.html', context)
...
def profil_etudiant(request, etudiant_id=None):
    ...
    try:
        ...
        if etudiant.enseignant_referent and not hasattr(etudiant.enseignant_referent, 'user'):
            etudiant.enseignant_referent = None
            etudiant.save()
    ...
    return render(request, 'Stages/profil_etudiant.html', {
        'user': etudiant.user,
        'etudiant': etudiant,
        'stages': stages,
        'enseignant_referent_name': etudiant.enseignant_referent.user.get_full_name() if etudiant.enseignant_referent else None
    })
