...
class Offre(models.Model):
    ...
    duree_semaines = models.PositiveIntegerField(help_text="Durée du stage en semaines")
    date_debut = models.DateField(
        verbose_name="Date de début prévisionnelle",
        null=True,
        blank=True,
        help_text="Date prévisionnelle de début du stage"
    )
...
class Stage(models.Model):
    ...
    date_fin_reelle = models.DateField(
        null=True, 
        blank=True,
        verbose_name="Date réelle de fin"
    )
    ...
    tuteur_entreprise = models.CharField(
        max_length=100,
    ...
    remuneration = models.DecimalField(
        ...
        null=True,
        blank=True,
        verbose_name="Rémunération mensuelle effective"
    )
    
    objectifs = models.TextField(
        verbose_name="Objectifs du stage",
        blank=True
    )
    ...
    tuteur_entreprise_telephone = models.CharField(
        max_length=20,
        blank=True,
    ...
    def __str__(self):
        return f"Stage de {self.candidature.etudiant.user.get_full_name()} chez {self.candidature.offre.entreprise.nom}"

    @property
    def est_termine(self):
        return self.statut == 'TERMINE' or (self.date_fin and self.date_fin < timezone.now().date())
    
    @property
    def est_en_cours(self):
        return self.statut == 'EN_COURS' and (self.date_debut <= timezone.now().date() <= self.date_fin)

    @property
    def duree_semaines(self):
        """Calcule la durée du stage en semaines complètes"""
        return (self.date_fin - self.date_debut).days // 7

    @property
    def jours_restants(self):
        """Nombre de jours restants avant la fin du stage"""
        aujourdhui = timezone.now().date()
        return (self.date_fin - aujourdhui).days if self.date_fin > aujourdhui else 0

    @property
    def pourcentage_complete(self):
        """Pourcentage d'avancement du stage"""
        total_days = (self.date_fin - self.date_debut).days
        if total_days <= 0:
            return 0
            
        days_passed = (timezone.now().date() - self.date_debut).days
        pourcentage = (days_passed / total_days) * 100
        return min(100, max(0, int(pourcentage)))
